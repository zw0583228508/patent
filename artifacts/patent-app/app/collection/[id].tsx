...
export default function CollectionDetailScreen() {
  ...
  const feedById = React.useMemo(() => {
    const map: Record<string, FeedItem> = {};
    for (const item of FEED_ITEMS) map[item.id] = item;
    return map;
  }, []);
  ...
  const load = useCallback(
    async (reset = false) => {
      if (!id) return;
      try {
        const res = await api.collections.getPosts(id, {
          limit: LIMIT,
          offset: reset ? 0 : offset,
        });
        setCollection(res.collection);
        const found = res.postIds
          .map((pid) => feedById[pid])
          .filter(Boolean) as FeedItem[];
        if (reset) {
          setItems(found);
          setOffset(res.postIds.length);
        } else {
          setItems((prev) => [...prev, ...found]);
          setOffset((o) => o + res.postIds.length);
        }
    ...
    >
      ...
      <FlatList
        ...
        renderItem={({ item, index }) =>
          item.type === "question" ? (
            <QuestionCard tip={item as any} index={index} />
          ) : (
            <TipCard tip={item as any} index={index} />
          )
        }
      />
    </View>
