...
export default function PostScreen() {
  ...
  const POST_TYPES = [
    { id: "tip", label: t("newTip"), icon: "zap", desc: t("shareKnowledge") },
    { id: "question", label: t("requestSolution"), icon: "help-circle", desc: t("communityHelps") },
  ];
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { paddingTop: topPad + 8, borderBottomColor: colors.border, flexDirection: dir }]}>
        ...
        <Text style={[styles.title, { color: colors.foreground }]}>{t("post")}</Text>
        <TouchableOpacity
          style={[styles.submitBtn, { backgroundColor: text.trim() ? colors.primary : colors.surface2 }]}
      ...
      <View style={styles.body}>
        <View style={[styles.typeRow, { flexDirection: dir }]}>
          {POST_TYPES.map((pt) => (
            ...
            >
              ...
              <View>
                <Text style={[styles.typeLabel, { color: postType === pt.id ? colors.primary : colors.foreground, textAlign }]}>
                  {pt.label}
                </Text>
                <Text style={[styles.typeDesc, { color: colors.mutedForeground, textAlign }]}>{pt.desc}</Text>
              </View>
            </TouchableOpacity>
          ))}
        </View>
        ...
        <TextInput
          ...
          multiline
          numberOfLines={6}
          placeholder={postType === "tip" ? t("newTip") + "..." : t("requestSolution") + "..."}
        ...
        <Text style={[styles.catLabel, { color: colors.mutedForeground, textAlign }]}>{t("category")}</Text>
        <View style={[styles.catGrid, { flexDirection: isRTL ? "row-reverse" : "row" }]}>
...
const styles = StyleSheet.create({
  ...
  typeRow: { gap: 10 },
  typeBtn: { flex: 1, alignItems: "center", borderRadius: 14, borderWidth: 1, padding: 14, gap: 10 },
  typeLabel: { fontSize: 14, fontWeight: "600" as const, marginTop: 2 },
  typeDesc: { fontSize: 11, marginTop: 2 },
  textArea: { borderRadius: 14, borderWidth: 1, padding: 14, fontSize: 15, minHeight: 140 },
  catLabel: { fontSize: 12, fontWeight: "600" as const },
  catGrid: { flexWrap: "wrap", gap: 8 },
  catChip: { alignItems: "center", borderRadius: 100, borderWidth: 1, paddingHorizontal: 12, paddingVertical: 7, gap: 6 },
